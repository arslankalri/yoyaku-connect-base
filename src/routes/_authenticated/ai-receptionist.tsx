...
const SCENARIOS = [
  ...
  { key: "recept.test.tomorrow", ja: "明日は何時まで営業していますか？", en: "How late are you open tomorrow?" },
  { key: "recept.test.colorStaff", ja: "カラーは誰が担当できますか？", en: "Which staff can do a colour?" },
...
function DataRow({
  ...
  return (
    <li className="flex items-center gap-2.5 text-sm">
      ...
      <span className={cn("shrink-0 text-xs font-medium", ready ? "text-foreground" : "text-muted-foreground")}>
        {value}
      </span>
    </li>
  );
          ...
          <div className="glass-panel p-4">
            ...
            <ul className="mt-3 space-y-2">
              ...
              <DataRow
                ...
                value={openDays > 0 ? t("recept.data.days").replace("{n}", String(openDays)) : "—"}
                ready={openDays > 0}
              />
              <DataRow
              ...
              <DataRow
                ...
                value={
                  activeStaff > 0 ? t("recept.data.people").replace("{n}", String(activeStaff)) : "—"
                }
                ready={activeStaff > 0}
              />
              <DataRow
