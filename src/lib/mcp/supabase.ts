...
/** The signed-in owner's business, or null when setup is incomplete. */
export async function currentBusiness(ctx: ToolContext) {
  const supabase = supabaseForUser(ctx);
  const { data, error } = await supabase
    .from("businesses")
    .select("id, name, phone, postal_code, address, website, timezone, business_type")
...
export function noBusiness() {
  return {
    content: [
      {
        type: "text" as const,
        text: "No business found for this account. Finish the setup wizard in the app first.",
      },
    ],
    isError: true,
  };
